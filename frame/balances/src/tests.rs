...
fn transferring_reserved_balance_to_nonexistent_should_fail() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(Balances::reserve(&1, 111));
		assert_noop!(
			Balances::repatriate_reserved(&1, &2, 42),
...
fn check_vesting_status() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				Balances::vesting_balance(&12),
				user12_free_balance - 256 * 5
			);
...
fn extra_balance_should_transfer() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(Balances::transfer((Some(2), None).into(), 3, 100)); // Account 2 can send extra units
			                                                    // gained
		});
}
