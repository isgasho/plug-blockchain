...
pub trait ChargeFee<AccountId> {
	...
	/// Refund `amount` of previous charged fees from `transactor`. Return Ok iff the refund was
	/// successful.
...
impl<T, U> ChargeExtrinsicFee<T, U> for DummyChargeFee<T, U> {
	fn charge_extrinsic_fee<'a>(_: &T, _: usize, _: &'a U) -> Result<(), &'static str> { Ok(()) }
}
...
/// When a doughnut proof is included along with a transaction, `verify` will be invoked just before
/// executing method logic.
pub trait DelegatedDispatchVerifier {
	...
	/// Check the doughnut authorizes a dispatched call from runtime to the specified contract
	...
	/// Check the doughnut authorizes a dispatched call from a contract to another contract with the
	/// specified addresses for this domain.
...
impl<D: PlugDoughnutApi, A: Parameter> DelegatedDispatchVerifier for DummyDispatchVerifier<D, A> {
	...
	fn verify_dispatch(_: &Self::Doughnut, _: &str, _: &str) -> Result<(), &'static str> { Ok(()) }
...
impl DelegatedDispatchVerifier for () {
	...
	fn verify_dispatch(
		...
		DummyDispatchVerifier::<Self::Doughnut, Self::AccountId>::verify_dispatch(
			doughnut, module, method,
		)
	}
	...
	fn verify_runtime_to_contract_call(
		...
		DummyDispatchVerifier::<Self::Doughnut, Self::AccountId>::verify_runtime_to_contract_call(
			caller, doughnut, addr,
		)
	}
	...
	fn verify_contract_to_contract_call(
		...
		DummyDispatchVerifier::<Self::Doughnut, Self::AccountId>::verify_contract_to_contract_call(
			caller, doughnut, addr,
		)
	}
}
...
/// It's main purpose is to allow checking if an `OuterOrigin` contains a doughnut (i.e. it is
/// delegated).
