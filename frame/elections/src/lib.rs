...
impl<T: Trait> Module<T> {
	...
	pub fn is_a_candidate(who: &T::AccountId) -> bool { <RegisterInfoOf<T>>::exists(who) }
	...
	fn do_set_approvals(
		...
		<VoterInfoOf<T>>::insert(&who, VoterInfo::<BalanceOf<T>> {
			last_active: index,
			last_win: index,
	...
	fn finalize_tally() -> Result {
		...
		incoming
			.iter()
			.filter_map(|i| Self::candidate_reg_info(i))
			.for_each(|r| {
