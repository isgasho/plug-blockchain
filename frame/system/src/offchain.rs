...
use codec::Encode;
use sp_runtime::{
	app_crypto::{self, RuntimeAppPublic},
...
impl<Public, Signature, AppPublic> Signer<Public, Signature> for AppPublic
where
	...
	Signature:
		From<<<AppPublic as RuntimeAppPublic>::Signature as app_crypto::AppSignature>::Generic>,
...
pub trait SubmitSignedTransaction<T: crate::Trait, Call> {
	...
	fn sign_and_submit(call: impl Into<Call>, public: PublicOf<T, Call, Self>) -> Result<(), ()> {
		...
		let (call, signature_data) =
			Self::CreateTransaction::create_transaction::<Self::Signer>(call, public, id, expected)
				.ok_or(())?;
