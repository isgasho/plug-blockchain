...
mod tests {
	...
	use contracts::{self, ContractAddressFor};
	use indices;
	use node_primitives::{Balance, BlockNumber, Hash};
	use node_runtime::{
		constants::currency::*, impls::LinearWeightToFee, Balances, Block, BuildStorage, Call,
		CheckedExtrinsic, Event, Header, Runtime, System, TransactionBaseFee, TransactionByteFee,
		TransactionPayment, TransferFee, UncheckedExtrinsic, WeightFeeCoefficient,
	};
	...
	use runtime_support::{
		...
		Hashable, StorageMap, StorageValue,
	};
	...
	// block 1 and 2 must be created together to ensure transactions are only signed once (since
	// they are not guaranteed to be deterministic) and to ensure that the correct state is
	// propagated from block1's execution to block2 to derive the correct storage_root.
	fn blocks() -> ((Vec<u8>, Hash), (Vec<u8>, Hash)) {
		let mut t = new_test_ext(COMPACT_CODE, false);
		let block1 = construct_block(&mut t, 1, GENESIS_HASH.into(), vec![
			CheckedExtrinsic {
				signed: None,
				function: Call::Timestamp(timestamp::Call::set(42 * 1000)),
			},
		...
		let block2 = construct_block(&mut t, 2, block1.1.clone(), vec![
			CheckedExtrinsic {
				signed: None,
		...
		t.execute_with(|| {
			...
			let events = vec![
				EventRecord {
					...
					event: Event::system(system::Event::ExtrinsicSuccess(DispatchInfo {
						weight: 10000,
...
"#;
	...
	fn deploying_wasm_contract_should_work() {
		...
		t.execute_with(|| {
			// Verify that the contract constructor worked well and code of TRANSFER contract is
			// actually deployed.
			assert_eq!(
	...
	fn full_native_block_import_works_with_changes_trie() {
		...
		assert!(t
			.ext()
			.storage_changes_root(&GENESIS_HASH.encode())
			.unwrap()
			.is_some());
	}
	...
	fn fee_multiplier_increases_and_decreases_on_big_weight() {
		...
		let block1 = construct_block(&mut tt, 1, GENESIS_HASH.into(), vec![
			CheckedExtrinsic {
				signed: None,
				function: Call::Timestamp(timestamp::Call::set(42 * 1000)),
			},
		...
		let block2 = construct_block(&mut tt, 2, block1.1.clone(), vec![
			CheckedExtrinsic {
				signed: None,
				function: Call::Timestamp(timestamp::Call::set(52 * 1000)),
			},
	...
	fn block_weight_capacity_report() {
		// Just report how many transfer calls you could fit into a block. The number should at
		// least be a few hundred (250 at the time of writing but can change over time). Runs until
		// panic.
	...
	fn block_length_capacity_report() {
		...
		loop {
			// NOTE: this is super slow. Can probably be improved.
			let block = construct_block(&mut tt, block_number, previous_hash, vec![
				CheckedExtrinsic {
					signed: None,
					function: Call::Timestamp(timestamp::Call::set(time * 1000)),
				},
