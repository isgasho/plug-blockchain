...
impl<C: Get<Balance>> Convert<Weight, Balance> for LinearWeightToFee<C> {
	fn convert(w: Weight) -> Balance {
		// substrate-node a weight of 10_000 (smallest non-zero weight) to be mapped to 10^7 units
		// of fees, hence:
...
mod tests {
	use super::*;
	use crate::{
		constants::currency::*, AvailableBlockRatio, MaximumBlockWeight, Runtime,
		TargetBlockFullness, TransactionPayment,
	};
	use sp_runtime::assert_eq_error_rate;
	use support::weights::Weight;

	fn max() -> Weight { MaximumBlockWeight::get() }

	fn target() -> Weight { TargetBlockFullness::get() * max() }
	...
	fn feemul(parts: i64) -> Fixed64 { Fixed64::from_parts(parts) }

	fn run_with_system_weight<F>(w: Weight, assertions: F)
	where
		F: Fn() -> (),
	{
		let mut t: runtime_io::TestExternalities = system::GenesisConfig::default()
	...
	fn empty_chain_simulation() {
		...
		run_with_system_weight(block_weight, || {
			...
			assert!(
				iterations > 50_000,
				"This assertion is just a warning; Don't panic. Current substrate/polkadot node \
				 are configured with a _slow adjusting fee_ mechanism. Hence, it is really \
				 unlikely that fees collapse to zero even on an empty chain in less than at least \
				 of couple of thousands of empty blocks. But this simulation indicates that fees \
				 collapsed to zero after {} almost-empty blocks. Check it",
				iterations,
			);
		})
	}
