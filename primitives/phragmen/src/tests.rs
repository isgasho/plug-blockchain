...
fn float_phragmen_poc_works() {
	...
	assert_eq_uvec!(assignments, vec![
		(10, vec![(2, 1.0)]),
		(20, vec![(3, 1.0)]),
		(30, vec![(2, 0.5), (3, 0.5)]),
	]);
	...
	assert_eq!(support_map.get(&2).unwrap(), &_Support {
		own: 0.0,
		total: 25.0,
		others: vec![(10u64, 10.0), (30u64, 15.0)]
	});
	assert_eq!(support_map.get(&3).unwrap(), &_Support {
		own: 0.0,
		total: 35.0,
...
fn phragmen_accuracy_on_large_scale_only_validators() {
	...
	assert_eq_uvec!(winners, vec![
		(1, 18446744073709551614u128),
		(5, 18446744073709551613u128)
	]);
...
fn phragmen_accuracy_on_large_scale_validators_and_nominators() {
	...
	assert_eq_uvec!(winners, vec![
		(2, 36893488147419103226u128),
		(1, 36893488147419103219u128)
	]);
...
fn phragmen_large_scale_test() {
	...
	assert_eq_uvec!(winners, vec![
		(24, 1490000000000200000u128),
		(22, 1490000000000100000u128)
	]);
	check_assignments(assignments);
}
