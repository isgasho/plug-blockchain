...
use crate::utils::{
	create_exchangeable_host_function_ident, create_host_function_ident, generate_crate_access,
	get_function_argument_names, get_function_argument_names_and_types_without_ref,
	get_function_argument_types, get_function_argument_types_ref_and_mut,
	get_function_argument_types_without_ref, get_function_arguments, get_trait_methods,
};

use syn::{
	spanned::Spanned, Error, Ident, ItemTrait, Pat, Result, ReturnType, Signature, TraitItem,
	TraitItemMethod,
};
...
pub fn generate(trait_def: &ItemTrait, is_wasm_only: bool) -> Result<TokenStream> {
	...
	let extern_host_function_impls =
...
fn generate_exchangeable_host_function(method: &TraitItemMethod) -> Result<TokenStream> {
	...
	let doc_string = format!(
		" Exchangeable host function used by [`{}`].",
		method.sig.ident
	);
	...
	match pat {
		Pat::Ident(pat_ident) => {
			if let Some(by_ref) = pat_ident.by_ref {
				...
				Ok(Ident::new(
					&format!("{}_ffi_value", pat_ident.ident),
					Span::call_site(),
				))
			}
		},
