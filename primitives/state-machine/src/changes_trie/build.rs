	...
	Number: BlockNumber,
{
	...
	let (extrinsics_input, children_extrinsics_input) =
		prepare_extrinsics_input(backend, &number, changes)?;
	...
	Number: BlockNumber,
{
	let (committed, prospective) = if let Some(sk) = storage_key.as_ref() {
		(
			changes.committed.children.get(sk),
			changes.prospective.children.get(sk),
		)
	} else {
	...
	committed
		...
		.try_fold(
			BTreeMap::new(),
			|mut map: BTreeMap<&[u8], (ExtrinsicIndex<Number>, Vec<u32>)>, (k, v)| {
				match map.entry(k) {
					Entry::Vacant(entry) => {
		...
		.map(|pairs| {
			pairs
				.into_iter()
				.map(|(_, (k, v))| InputPair::ExtrinsicIndex(k, v))
		})
}
	...
	digest_input_blocks
		...
		.try_fold(
			...
			move |(mut map, mut child_map), digest_build_block| {
				...
				let trie_root = trie_root.ok_or_else(|| {
					format!(
						"No changes trie root for block {}",
						digest_build_block.clone()
					)
				})?;
		...
		.map(|(pairs, child_pairs)| {
			(
				pairs
					.into_iter()
					.map(|(_, (k, v))| InputPair::DigestIndex(k, v)),
				child_pairs
					.into_iter()
					.map(|(sk, pairs)| {
						(
...
mod test {
	...
	use crate::{
		...
		overlayed_changes::{OverlayedChangeSet, OverlayedValue},
	};
	use codec::Encode;
	use primitives::{storage::well_known_keys::EXTRINSIC_INDEX, Blake2Hasher};
	...
	fn prepare_for_build(
		...
		let storage = InMemoryStorage::with_inputs(
			vec![
				...
				(zero + 2, vec![InputPair::ExtrinsicIndex(
					ExtrinsicIndex {
						block: zero + 2,
						key: vec![102],
					},
				...
				(zero + 5, Vec::new()),
				(zero + 6, vec![InputPair::ExtrinsicIndex(
					ExtrinsicIndex {
						block: zero + 6,
						key: vec![105],
					},
				...
				(zero + 7, Vec::new()),
				(zero + 8, vec![InputPair::DigestIndex(
					DigestIndex {
						block: zero + 8,
						key: vec![105],
					},
		...
		let changes = OverlayedChanges {
			prospective: OverlayedChangeSet {
				...
				children: vec![
					(
						child_trie_key1.clone(),
						vec![(vec![100], OverlayedValue {
							value: Some(vec![200]),
							extrinsics: Some(vec![0, 2].into_iter().collect()),
						})]
						.into_iter()
	...
	fn cache_is_used_when_changes_trie_is_built() {
		...
		let cached_data4 = IncompleteCacheAction::CacheBuildData(IncompleteCachedBuildData::new())
			...
			.insert(
				Some(b"1".to_vec()),
				vec![vec![103], vec![104]].into_iter().collect(),
			)
			.insert(
				Some(b"2".to_vec()),
				vec![vec![105], vec![106]].into_iter().collect(),
			)
