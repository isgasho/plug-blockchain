	...
	fn prepare_for_drilldown() -> (Configuration, InMemoryStorage<Blake2Hasher, u64>) {
		...
		let backend = InMemoryStorage::with_inputs(
			vec![
				...
				(1, vec![]),
				(2, vec![]),
				(3, vec![InputPair::ExtrinsicIndex(
					ExtrinsicIndex {
						block: 3,
						key: vec![42],
					},
				...
				(5, vec![]),
				(6, vec![InputPair::ExtrinsicIndex(
					ExtrinsicIndex {
						block: 6,
						key: vec![42],
					},
			...
			vec![(b"1".to_vec(), vec![
				(1, vec![InputPair::ExtrinsicIndex(
					ExtrinsicIndex {
						block: 1,
						key: vec![42],
					},
				...
				(2, vec![InputPair::ExtrinsicIndex(
					ExtrinsicIndex {
						block: 2,
						key: vec![42],
					},
