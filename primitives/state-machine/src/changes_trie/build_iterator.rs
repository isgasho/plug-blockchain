...
pub fn digest_build_iterator<'a, Number: BlockNumber>(
	...
	let (_, _, digest_step) = match config
		.config
		.digest_level_at_block(config.zero, block.clone())
	{
...
impl<Number: BlockNumber> DigestBuildIterator<Number> {
	...
	pub fn empty() -> Self { Self::new(Zero::zero(), Zero::zero(), 0, 0) }
}
...
mod tests {
	...
	fn suggest_digest_inclusion_returns_empty_iterator() {
		fn test_with_zero(zero: u64) {
			let empty = (0, 0, 0);
			assert_eq!(
				digest_build_iterator_basic(4, 16, zero, zero + 0),
				empty,
				"block is 0"
			);
			assert_eq!(
