...
use crate::{
	changes_trie::{
		...
		AnchorBlockId, BlockNumber, Configuration, Storage,
	},
	...
	H::Out: Codec,
{
	...
	let (first, last) =
		match pruning_range(config, min_blocks_to_keep, current_block.number.clone()) {
...
mod tests {
	...
	fn prune_works() {
		fn prepare_storage() -> InMemoryStorage<Blake2Hasher, u64> {
			...
			let root2 = insert_into_memory_db::<Blake2Hasher, _>(&mut mdb2, vec![
				(vec![11], vec![21]),
				(vec![12], vec![22]),
			])
			.unwrap();
			...
			let ch_root3 =
				insert_into_memory_db::<Blake2Hasher, _>(&mut mdb3, vec![(vec![110], vec![120])])
					.unwrap();
	...
	fn pruning_range_works() {
		...
		assert_eq!(
			pruning_range(&config(10, 0), 100u64, 110u64),
			Some((10, 10))
		);
		assert_eq!(
			pruning_range(&config(10, 0), 100u64, 210u64),
			Some((110, 110))
		);
		...
		assert_eq!(
			pruning_range(&config(::std::u32::MAX, 2), 1u64, 1024u64),
			None
		);
		assert_eq!(
			pruning_range(&config(::std::u32::MAX, 2), ::std::u64::MAX, 1024u64),
			None
		);
		...
		assert_eq!(
			pruning_range(&config(32, 2), 4096u64, 5120u64),
			Some((1, 1024))
		);
		assert_eq!(
			pruning_range(&config(32, 2), 4096u64, 6144u64),
			Some((1025, 2048))
		);
	}
	...
	fn oldest_non_pruned_trie_works() {
		...
		assert_eq!(
			oldest_non_pruned_trie(&config(100, 2), 100u64, 20110u64),
			10001
		);
	}
}
