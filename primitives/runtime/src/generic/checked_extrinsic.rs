...
use crate::{
	traits::{
		self, Dispatchable, MaybeDisplay, MaybeDoughnut, Member, PlugDoughnutApi, SignedExtension,
	},
...
impl<AccountId, Call, Extra, Origin, Info, Doughnut> traits::Applyable
	...
	Doughnut: Member + PlugDoughnutApi<PublicKey = AccountId>,
{
	...
	fn sender(&self) -> Option<&Self::AccountId> { self.signed.as_ref().map(|x| &x.0) }
