...
impl ChangesTrieConfiguration {
	...
	where
		Number: From<u32>
			+ PartialEq
			+ ::rstd::ops::Rem<Output = Number>
			+ ::rstd::ops::Sub<Output = Number>
			+ ::rstd::cmp::PartialOrd
			+ Zero,
	{
		block > zero
	...
	where
		Number: Clone
			...
			+ PartialOrd
			+ PartialEq
			+ ::rstd::ops::Add<Output = Number>
			+ ::rstd::ops::Sub<Output = Number>
			+ ::rstd::ops::Div<Output = Number>
			+ ::rstd::ops::Mul<Output = Number>
			+ Zero,
	{
		if block <= zero {
			return None
			...
			+ ::rstd::ops::Mul<Output = Number>,
	{
		...
		Some(
			if block == last_max_digest_block {
...
mod tests {
	...
	fn digest_level_at_block_works() {
		fn test_with_zero(zero: u64) {
			...
			assert_eq!(
				config(8, 4).digest_level_at_block(zero, zero + 8u64),
				Some((1, 8, 1))
			);
			assert_eq!(
				config(8, 4).digest_level_at_block(zero, zero + 64u64),
				Some((2, 64, 8))
			);
			assert_eq!(
	...
	fn max_digest_interval_works() {
		...
		assert_eq!(
			config(::std::u32::MAX, 1024).max_digest_interval(),
			::std::u32::MAX
		);
	}
	...
	fn next_max_level_digest_range_works() {
		...
		assert_eq!(
			config(2, 1).next_max_level_digest_range(0u64, 16),
			Some((15, 16))
		);
		assert_eq!(
			config(4, 1).next_max_level_digest_range(0u64, 16),
			Some((13, 16))
		);
		assert_eq!(
			config(32, 1).next_max_level_digest_range(0u64, 16),
			Some((1, 32))
		);
		assert_eq!(
			config(2, 3).next_max_level_digest_range(0u64, 10),
			Some((9, 16))
		);
		assert_eq!(
			config(2, 3).next_max_level_digest_range(0u64, 8),
			Some((1, 8))
		);
		assert_eq!(
			config(2, 1).next_max_level_digest_range(1u64, 1),
			Some((2, 3))
		);
		assert_eq!(
			config(2, 2).next_max_level_digest_range(7u64, 9),
			Some((8, 11))
		);

		assert_eq!(
			config(2, 2).next_max_level_digest_range(7u64, 5),
			Some((8, 11))
		);
	}
