...
impl UncheckedFrom<[u8; 64]> for Public {
	fn unchecked_from(x: [u8; 64]) -> Self { Public(x) }
}
...
impl rstd::hash::Hash for Public {
	fn hash<H: rstd::hash::Hasher>(&self, state: &mut H) { self.0.hash(state); }
}
...
impl Signature {
	...
	pub fn from_raw(data: [u8; 65]) -> Signature { Signature(data) }
...
impl Public {
	...
	pub fn from_raw(data: [u8; 64]) -> Self { Public(data) }
	...
	pub fn as_array_ref(&self) -> &[u8; 64] { self.as_ref() }
}
...
impl TraitPair for Pair {
	...
	/// Get the public key.
	fn public(&self) -> Public { Public::from_full(self.public.serialize()) }
	...
	fn to_raw_vec(&self) -> Vec<u8> { self.seed().to_vec() }
}

#[cfg(feature = "full_crypto")]
impl Pair {
	/// Get the seed for this key.
	pub fn seed(&self) -> Seed { self.secret.serialize() }
