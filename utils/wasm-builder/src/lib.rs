...
//! `WASM_BINARY_BLOATY`. The former is a compact WASM binary and the latter is not compacted.
//!
...
//!   recompiled.
//! - `BUILD_DUMMY_WASM_BINARY` - Builds dummy wasm binaries. These dummy binaries are empty and
//!   useful for `cargo check` runs.
...
//!   build.
//! - `TRIGGER_WASM_BUILD` - Can be set to trigger a wasm build. On subsequent calls the value of
//!   the variable needs to change. As WASM builder instructs `cargo` to watch for file changes this
...
fn check_skip_build() -> bool { env::var(SKIP_BUILD_ENV).is_ok() }
