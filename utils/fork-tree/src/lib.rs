...
impl<E: std::error::Error> fmt::Display for Error<E> {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		let message = match *self {
			...
			Error::Revert => "Tried to import or finalize node that is an ancestor of a \
...
impl<E: std::error::Error> From<E> for Error<E> {
	fn from(err: E) -> Error<E> { Error::Client(err) }
}
	...
	V: Clone,
{
	...
	where
		...
		P: Fn(&V) -> bool,
	{
		let new_root = self.find_node_where(hash, number, is_descendent_of, predicate)?;
			...
			root.children = children
				...
				.filter(|node| {
					node.number == *number && node.hash == *hash
						|| node.number < *number
							&& is_descendent_of(&node.hash, hash).unwrap_or(false)
				})
				.take(1)
				.collect();
...
mod test {
	...
	fn prune_works() {
		...
		tree.prune(&"C", &3, &is_descendent_of, &|_| true).unwrap();
		...
		tree.prune(&"E", &5, &is_descendent_of, &|_| true).unwrap();
