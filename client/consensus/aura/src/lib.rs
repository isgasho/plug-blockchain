...
use client::{blockchain::ProvideCache, BlockOf};
...
use sp_runtime::{
	...
	traits::{Block as BlockT, DigestItemFor, Header, Member, ProvideRuntimeApi, Zero},
	Justification,
};
...
impl<H, B, C, E, I, P, Error, SO> slots::SimpleSlotWorker<B> for AuraWorker<C, E, I, P, SO>
where
	B: BlockT<Header = H>,
	...
	Error: ::std::error::Error + Send + From<::consensus_common::Error> + From<I::Error> + 'static,
{
	...
	fn logging_target(&self) -> &'static str { "aura" }
	...
	fn authorities_len(&self, epoch_data: &Self::EpochData) -> usize { epoch_data.len() }
	...
	fn claim_slot(
		...
		expected_author.and_then(|p| {
			self.keystore
				.read()
				.key_pair_by_type::<P>(&p, app_crypto::key_types::AURA)
				.ok()
		})
	}
...
impl<H, B: BlockT, C, E, I, P, Error, SO> SlotWorker<B> for AuraWorker<C, E, I, P, SO>
where
	B: BlockT<Header = H>,
...
/// will be returned. if it's successful, returns the pre-header and the digest item containing the
/// seal.
///
...
#[forbid(deprecated)]
impl<B: BlockT, C, P, T> Verifier<B> for AuraVerifier<C, P, T>
where
	...
	C::Api: AuraApi<B, A>,
{
	...
	let map_err = |error| {
		consensus_common::Error::from(consensus_common::Error::ClientImport(format!(
...
mod tests {
	...
	use network::{
		...
		test::{Block as TestBlock, PeersClient, PeersFullClient, *},
	};
	use parking_lot::Mutex;
	...
	impl TestNetFactory for AuraTestNet {
		...
		fn from_config(_config: &ProtocolConfig) -> Self { AuraTestNet { peers: Vec::new() } }
		...
		fn peers(&self) -> &Vec<Peer<Self::PeerData, DummySpecialization>> { &self.peers }
	...
	fn authoring_blocks() {
		...
		for (peer_id, key) in peers {
			...
			let keystore =
				keystore::Store::open(keystore_path.path(), None).expect("Creates keystore.");
