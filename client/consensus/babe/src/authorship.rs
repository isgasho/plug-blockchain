...
use babe_primitives::{
	AuthorityId, AuthorityPair, BabeAuthorityWeight, BabeConfiguration, BabePreDigest, Epoch,
	SlotNumber, BABE_ENGINE_ID, BABE_VRF_PREFIX,
};
use codec::Encode;
use keystore::KeyStorePtr;
use merlin::Transcript;
use primitives::{blake2_256, Pair, U256};
...
fn claim_secondary_slot(
	...
	let expected_author =
		super::authorship::secondary_slot_author(slot_number, authorities, randomness)?;

	let keystore = keystore.read();

	for (pair, authority_index) in authorities.iter().enumerate().flat_map(|(i, a)| {
		keystore
			.key_pair::<AuthorityPair>(&a.0)
			.ok()
			.map(|kp| (kp, i))
	}) {
...
fn claim_primary_slot(
	...
	for (pair, authority_index) in authorities.iter().enumerate().flat_map(|(i, a)| {
		...
		let threshold =
			super::authorship::calculate_primary_threshold(c, authorities, authority_index);
