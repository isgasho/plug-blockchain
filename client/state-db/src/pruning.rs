...
fn to_journal_key(block: u64) -> Vec<u8> { to_meta_key(PRUNING_JOURNAL, &block) }
...
impl<BlockHash: Hash, Key: Hash> RefWindow<BlockHash, Key> {
	...
	pub fn window_size(&self) -> u64 { (self.death_rows.len() - self.pending_prunings) as u64 }
	...
	pub fn mem_used(&self) -> usize { 0 }

	pub fn pending(&self) -> u64 { self.pending_number + self.pending_prunings as u64 }
