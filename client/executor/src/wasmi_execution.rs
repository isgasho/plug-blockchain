...
use wasmi::{
	memory_units::Pages,
	ImportsBuilder, MemoryInstance, MemoryRef, Module, ModuleInstance, ModuleRef,
...
impl<'a> sandbox::SandboxCapabilities for FunctionExecutor<'a> {
	...
	fn allocate(&mut self, len: WordSize) -> Result<Pointer<u8>, Error> {
		...
		self.memory
			.with_direct_access_mut(|mem| heap.allocate(mem, len))
	}

	fn deallocate(&mut self, ptr: Pointer<u8>) -> Result<(), Error> {
		let heap = &mut self.heap;
		self.memory
			.with_direct_access_mut(|mem| heap.deallocate(mem, ptr))
	}
...
impl<'a> wasmi::Externals for FunctionExecutor<'a> {
	fn invoke_index(
		...
		let function = self.host_functions.get(index).ok_or_else(|| {
			Error::from(format!(
				"Could not find host function with index: {}",
				index
			))
		})?;
...
fn call_in_wasm_module(
	...
	let result = externalities::set_and_run_with_externalities(ext, || {
...
struct StateSnapshot {
	/// The offset and the content of the memory segments that should be used to restore the
	/// snapshot
...
impl WasmRuntime for WasmiRuntime {
	...
	fn call(
		...
		self.state_snapshot.apply(&self.instance).map_err(|e| {
...
pub fn create_instance(
	...
	let state_snapshot = StateSnapshot::take(&instance, data_segments, heap_pages).expect(
...
fn extract_data_segments(wasm_code: &[u8]) -> Result<Vec<DataSegment>, WasmError> {
	let raw_module: RawModule =
		deserialize_buffer(wasm_code).map_err(|_| WasmError::CantDeserializeWasm)?;
