...
fn execute_sandboxed(
	...
	fn env_inc_counter(
		...
		Ok(sandbox::ReturnValue::Value(sandbox::TypedValue::I32(
			e.counter as i32,
		)))
	}
	...
	let env_builder = {
		...
		let memory = match sandbox::Memory::new(1, Some(16)) {
			...
			Err(_) => unreachable!(
				"
				Memory::new() can return Err only if parameters are borked; We passing params here explicitly and \
				 they're correct; Memory::new() can't return a Error qed"
			),
		};
