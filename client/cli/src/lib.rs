...
use network::{
	self,
	config::{
		build_multiaddr, NetworkConfiguration, NodeKeyConfig, NonReservedPeerMode, TransportConfig,
	},
	multiaddr::Protocol,
};
use primitives::H256;
...
use std::{
	fs::{self, File},
	io::{stdin, stdout, Cursor, ErrorKind, Read, Seek, Write},
	iter,
...
use params::{
	BuildSpecCmd, CheckBlockCmd, Cors, ExportBlocksCmd, ImportBlocksCmd, MergeParameters,
	NetworkConfigurationParams, NodeKeyParams, NodeKeyType, PurgeChainCmd, RevertCmd, RunCmd,
...
fn load_spec<F, G, E>(cli: &SharedParams, factory: F) -> error::Result<ChainSpec<G, E>>
where
	G: RuntimeGenesis,
...
fn base_path(cli: &SharedParams, version: &VersionInfo) -> PathBuf {
	cli.base_path.clone().unwrap_or_else(|| {
		app_dirs::get_app_root(AppDataType::UserData, &AppInfo {
	...
	<I as IntoIterator>::Item: Into<std::ffi::OsString> + Clone,
{
	...
	match cli_args {
		...
		params::CoreParams::CheckBlock(params) => {
			ParseAndPrepare::CheckBlock(CheckBlock { params, version })
		},
		params::CoreParams::PurgeChain(params) => {
			ParseAndPrepare::PurgeChain(ParseAndPreparePurge { params, version })
		},
		params::CoreParams::Revert(params) => {
			ParseAndPrepare::RevertChain(ParseAndPrepareRevert { params, version })
		},
...
impl<'a> ParseAndPrepareBuildSpec<'a> {
	...
	pub fn run<C, G, S, E>(self, spec_factory: S) -> error::Result<()>
	where
		...
		E: ChainSpecExtension,
	{
		...
		if spec.boot_nodes().is_empty() && !self.params.disable_default_bootnode {
			...
			let cfg = service::Configuration::<C, _, _>::default_with_spec_and_base_path(
				spec.clone(),
				Some(base_path),
			);
			let node_key = node_key_config(
				...
				&Some(
					cfg.in_chain_config_dir(DEFAULT_NETWORK_CONFIG_PATH)
...
impl<'a> ParseAndPrepareExport<'a> {
	...
	{
		let config =
			create_config_with_db_path(spec_factory, &self.params.shared_params, self.version)?;
...
impl<'a> ParseAndPrepareImport<'a> {
		...
		Exit: IntoExit,
	{
		...
		let mut import_fut = builder(config)?.import_blocks(file, false).compat();
		let fut = futures::future::poll_fn(|cx| {
			if exit_recv.try_recv().is_ok() {
				return Poll::Ready(Ok(()))
			}
...
impl<'a> CheckBlock<'a> {
		...
		Exit: IntoExit,
	{
		...
		let input = if self.params.input.starts_with("0x") {
			&self.params.input[2..]
		} else {
			&self.params.input[..]
		};
		...
		let check = builder(config)?.check_block(block_id).compat();
		let mut runtime = tokio::runtime::Runtime::new().unwrap();
...
impl<'a> ParseAndPreparePurge<'a> {
	...
	pub fn run<G, E, S>(self, spec_factory: S) -> error::Result<()>
	where
		S: FnOnce(&str) -> Result<Option<ChainSpec<G, E>>, String>,
		G: RuntimeGenesis,
		E: ChainSpecExtension,
...
fn input_keystore_password() -> Result<String, String> {
	rpassword::read_password_from_tty(Some("Keystore password: ")).map_err(|e| format!("{:?}", e))
}
		...
		match &cli.pruning {
			...
			Some(s) => {
				if role == service::Roles::AUTHORITY && !cli.unsafe_pruning {
					return Err(error::Error::Input(
						"Validators should run with state pruning disabled (i.e. archive). You \
						 can ignore this check with `--unsafe-pruning`."
							.to_string(),
					))
				}
	...
	S: FnOnce(&str) -> Result<Option<ChainSpec<G, E>>, String>,
{
	...
	match is_node_name_valid(&config.name) {
		Ok(_) => (),
		Err(msg) => Err(error::Error::Input(format!(
	...
	config.keystore_path = cli
		.keystore_path
		.or_else(|| config.in_chain_config_dir(DEFAULT_KEYSTORE_CONFIG_PATH));
	...
	config.dev_key_seed = cli.keyring.account.map(|a| format!("//{}", a)).or_else(|| {
		if is_dev {
			Some("//Alice".into())
		} else {
			None
	...
	let grafana_interface: &str = if cli.grafana_external {
		"0.0.0.0"
	} else {
		"127.0.0.1"
	};
	...
	config.grafana_port = Some(parse_address(
		&format!("{}:{}", grafana_interface, 9955),
		cli.grafana_port,
	)?);
	...
	S: FnOnce(&str) -> Result<Option<ChainSpec<G, E>>, String>,
{
	...
	let mut config =
		service::Configuration::default_with_spec_and_base_path(spec.clone(), Some(base_path));
