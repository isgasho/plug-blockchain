...
use std::{cmp::Ord, fmt::Debug, ops::Add, sync::Arc};
...
impl<H, N> SharedAuthoritySet<H, N> {
	...
	pub(crate) fn inner(&self) -> &RwLock<AuthoritySet<H, N>> { &*self.inner }
}
	...
	H: Clone + Debug,
{
	...
	pub(crate) fn current_limit(&self) -> Option<N> { self.inner.read().current_limit() }

	/// Get the current set ID. This is incremented every time the set changes.
	pub(crate) fn set_id(&self) -> u64 { self.inner.read().set_id }
	...
	H: Clone + Debug,
{
		...
		F: Fn(&H, &H) -> Result<bool, E>,
	{
		...
		{
			// check if the given best block is in the same branch as the block that signaled the
			// change.
			if is_descendent_of(&change.canon_hash, &best_hash)? {
				...
				let median_last_finalized = match change.delay_kind {
					...
					_ => unreachable!(
						"pending_forced_changes only contains forced changes; forced changes have \
						 delay kind Best; qed."
					),
				};
...
impl<H, N: Add<Output = N> + Clone> PendingChange<H, N> {
	...
	pub fn effective_number(&self) -> N { self.canon_height.clone() + self.delay.clone() }
}
...
mod tests {
	...
	fn changes_iterated_in_pre_order() {
		...
		assert_eq!(authorities.pending_changes().collect::<Vec<_>>(), vec![
			&change_b, &change_a, &change_c, &change_e, &change_d
		],);
	}
	...
	fn disallow_multiple_changes_being_finalized_at_once() {
		...
		let status = authorities
			.apply_standard_changes("hash_b", 15, &is_descendent_of)
			.unwrap();
		assert!(status.changed);
		...
		let status = authorities
			.apply_standard_changes("hash_d", 40, &is_descendent_of)
			.unwrap();
		assert!(status.changed);
	...
	fn forced_changes() {
		...
		authorities
			...
			.unwrap();
		authorities
			.add_pending_change(change_b, &static_is_descendent_of(false))
			.unwrap();
		...
		assert!(authorities
			.add_pending_change(change_c, &is_descendent_of_a)
			.is_err());

		// too early.
		assert!(authorities
			.apply_forced_changes("hash_a10", 10, &static_is_descendent_of(true))
			.unwrap()
			.is_none());

		// too late.
		assert!(authorities
			.apply_forced_changes("hash_a16", 16, &static_is_descendent_of(true))
			.unwrap()
			.is_none());
		...
		assert_eq!(
			authorities
				.apply_forced_changes("hash_a15", 15, &is_descendent_of_a)
				.unwrap()
				.unwrap(),
			(42, AuthoritySet {
