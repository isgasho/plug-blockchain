...
pub type Commit<Block> =
	grandpa::Commit<<Block as BlockT>::Hash, NumberFor<Block>, AuthoritySignature, AuthorityId>;
...
type CommunicationInH<Block, H> =
	grandpa::voter::CommunicationIn<H, NumberFor<Block>, AuthoritySignature, AuthorityId>;
...
type CommunicationOutH<Block, H> =
	grandpa::voter::CommunicationOut<H, NumberFor<Block>, AuthoritySignature, AuthorityId>;
...
impl From<GrandpaError> for Error {
	fn from(e: GrandpaError) -> Self { Error::Grandpa(e) }
}
...
impl<Block, N> BlockSyncRequester<Block> for NetworkBridge<Block, N>
where
	Block: BlockT,
...
impl<H, N> From<Error> for CommandOrError<H, N> {
	fn from(e: Error) -> Self { CommandOrError::Error(e) }
}

impl<H, N> From<ClientError> for CommandOrError<H, N> {
	fn from(e: ClientError) -> Self { CommandOrError::Error(Error::Client(e)) }
}

impl<H, N> From<grandpa::Error> for CommandOrError<H, N> {
	fn from(e: grandpa::Error) -> Self { CommandOrError::Error(Error::from(e)) }
}

impl<H, N> From<VoterCommand<H, N>> for CommandOrError<H, N> {
	fn from(e: VoterCommand<H, N>) -> Self { CommandOrError::VoterCommand(e) }
}
...
impl<B, E, Block: BlockT<Hash = H256>, RA> GenesisAuthoritySetProvider<Block>
	for Client<B, E, Block, RA>
where
...
) -> sp_blockchain::Result<impl Future<Item = (), Error = ()> + Send + 'static>
where
	Block::Hash: Ord,
	B: Backend<Block, Blake2Hasher> + 'static,
	E: CallExecutor<Block, Blake2Hasher> + Send + Sync + 'static,
	...
	X: futures03::Future<Output = ()> + Clone + Send + Unpin + 'static,
{
		...
		let events = telemetry_on_connect
			.for_each(move |_| {
				...
				let maybe_authority_id =
					authority_id(&mut auths, &conf.keystore).unwrap_or(Default::default());
	...
	let telemetry_task = telemetry_task.then(|_| futures::future::empty::<(), ()>());

	use futures03::{FutureExt, TryFutureExt};

	Ok(voter_work
		.select(on_exit.map(Ok).compat())
	...
	VR: VotingRule<Block, Client<B, E, Block, RA>> + Clone + 'static,
{
	...
	fn rebuild_voter(&mut self) {
		...
		match &*self.env.voter_set_state.read() {
			...
			VoterSetState::Paused { .. } => self.voter = Box::new(futures::empty()),
		};
	}
...
where
	...
	N::In: Send + 'static,
{
	register_finality_tracker_inherent_data_provider(client, inherent_data_providers)?;
...
fn authority_id<'a, I>(authorities: &mut I, keystore: &Option<KeyStorePtr>) -> Option<AuthorityId>
where
	I: Iterator<Item = &'a AuthorityId>,
{
	match keystore {
		Some(keystore) => authorities.find_map(|p| {
			keystore
				.read()
				.key_pair::<AuthorityPair>(&p)
				.ok()
				.map(|ap| ap.public())
		}),
		None => None,
