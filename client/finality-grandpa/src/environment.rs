...
use client_api::{
	backend::Backend, call_executor::CallExecutor, utils::is_descendent_of, BlockchainEvents,
	Finalizer,
};
use grandpa::{
...
use fg_primitives::{AuthorityId, AuthoritySignature, RoundNumber, SetId};
...
impl<Block: BlockT> CompletedRounds<Block> {
	...
	pub fn set_info(&self) -> (SetId, &[AuthorityId]) { (self.set_id, &self.voters[..]) }

	/// Iterate over all completed rounds.
	pub fn iter(&self) -> impl Iterator<Item = &CompletedRound<Block>> { self.rounds.iter().rev() }
...
impl<Block: BlockT> HasVoted<Block> {
	...
	pub fn can_propose(&self) -> bool { self.propose().is_none() }

	/// Returns true if the voter can still prevote, false otherwise.
	pub fn can_prevote(&self) -> bool { self.prevote().is_none() }

	/// Returns true if the voter can still precommit, false otherwise.
	pub fn can_precommit(&self) -> bool { self.precommit().is_none() }
}
...
impl<Block: BlockT> From<VoterSetState<Block>> for SharedVoterSetState<Block> {
	fn from(set_state: VoterSetState<Block>) -> Self { SharedVoterSetState::new(set_state) }
}
...
impl<Block: BlockT<Hash = H256>, B, E, N, RA, SC, VR> grandpa::Chain<Block::Hash, NumberFor<Block>>
				...
				let target_header = if let Some(target_number) = limit {
					...
					loop {
						if *target_header.number() < target_number {
							unreachable!(
								"we are traversing backwards from a known block; blocks are \
								 stored contiguously; qed"
							);
						}
...
impl<B, E, Block: BlockT<Hash = H256>, N, RA, SC, VR>
	voter::Environment<Block::Hash, NumberFor<Block>> for Environment<B, E, Block, N, RA, SC, VR>
where
	Block: 'static,
	B: Backend<Block, Blake2Hasher> + 'static,
	...
	RA: Send + Sync,
{
	...
	if number <= status.finalized_number && client.hash(number)? == Some(hash) {
		// This can happen after a forced change (triggered by the finality tracker when finality is
		// stalled), since the voter will be restarted at the median last finalized block, which can
		// be lower than the local best finalized block.
