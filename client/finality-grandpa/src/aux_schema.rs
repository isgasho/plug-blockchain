...
use fg_primitives::{AuthorityList, RoundNumber, SetId};
...
use crate::{
	...
	consensus_changes::{ConsensusChanges, SharedConsensusChanges},
	environment::{
	...
	G: FnOnce() -> ClientResult<AuthorityList>,
{
	...
	match version {
					...
					match load_decode::<_, VoterSetState<Block>>(backend, SET_STATE_KEY)? {
						...
						None => {
							...
							let base = state.prevote_ghost.expect(
								"state is for completed round; completed rounds must have a \
								 prevote ghost; qed.",
							);

							VoterSetState::live(set.current().0, &set, base)
						},
					};
		...
		Some(other) => {
			return Err(
				ClientError::Backend(format!("Unsupported GRANDPA DB version: {:?}", other)).into(),
			)
		},
	}
...
mod test {
	...
	fn load_decode_from_v0_migrates_data_format() {
		...
		assert_eq!(*authority_set.inner().read(), AuthoritySet {
			current_authorities: authorities.clone(),
		...
		assert_eq!(&*set_state.read(), &VoterSetState::Live {
			completed_rounds: CompletedRounds::new(
				CompletedRound {
					number: round_number,
