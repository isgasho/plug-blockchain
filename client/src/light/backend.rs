...
use crate::{
	in_mem::{self, check_genesis_storage},
	light::blockchain::Blockchain,
};
use client_api::{
	backend::{
		AuxStore, Backend as ClientBackend, BlockImportOperation, ChildStorageCollection,
		NewBlockState, RemoteBackend, StorageCollection,
	},
...
use state_machine::{
	backend::InMemory as InMemoryState, Backend as StateBackend, ChangesTrieTransaction,
	TrieBackend,
};
use trie::MemoryDB;
...
impl<S, H: Hasher> Backend<S, H> {
	...
	pub fn blockchain(&self) -> &Arc<Blockchain<S>> { &self.blockchain }
}
	...
	H::Out: Ord,
{
	...
	fn blockchain(&self) -> &Blockchain<S> { &self.blockchain }

	fn used_state_cache_size(&self) -> Option<usize> { None }

	fn changes_trie_storage(&self) -> Option<&Self::ChangesTrieStorage> { None }
	...
	H::Out: Ord + codec::Codec,
{
	...
	fn next_child_storage_key(
		...
		match *self {
			GenesisOrUnavailableState::Genesis(ref state) => Ok(state
				.next_child_storage_key(storage_key, key)
				.expect(IN_MEMORY_EXPECT_PROOF)),
			GenesisOrUnavailableState::Unavailable => Err(ClientError::NotAvailableOnLightClient),
		}
	}
...
mod tests {
	...
	fn local_state_is_created_when_genesis_state_is_available() {
		...
		let backend: Backend<_, Blake2Hasher> =
	...
	fn light_aux_store_is_updated_via_non_importing_op() {
		...
		BlockImportOperation::<Block, Blake2Hasher>::insert_aux(&mut op, vec![(
			vec![1],
			Some(vec![2]),
		)])
		.unwrap();
