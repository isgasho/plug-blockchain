...
use consensus::{
	self, BlockCheckParams, BlockImportParams, BlockOrigin, BlockStatus, Error as ConsensusError,
...
use primitives::{
	...
	traits::CodeExecutor,
	Blake2Hasher, ChangesTrieConfiguration, ExecutionContext, NativeOrEncoded, NeverNativeValue,
	H256,
};
...
use sp_runtime::{
	...
	traits::{
		ApiRef, Block as BlockT, DigestFor, Header as HeaderT, NumberFor, One, ProvideRuntimeApi,
		SaturatedConversion, Zero,
	},
	BuildStorage, Justification,
};
use state_machine::{
	key_changes, key_changes_proof, merge_storage_proofs, prove_child_read, prove_read,
	Backend as StateBackend, BackendTrustLevel, ChangesTrieAnchorBlockId,
	ChangesTrieConfigurationRange, ChangesTrieRootsStorage, ChangesTrieStorage,
	ChangesTrieTransaction, DBValue, ExecutionManager, ExecutionStrategy, OverlayedChanges,
	StorageProof,
};
use std::{
	cell::RefCell,
...
use sp_api::{CallRuntimeAt, ConstructRuntimeApi, Core as CoreApi, InitializeBlock, ProofRecorder};

pub use client_api::{
	backend::{
		self, BlockImportOperation, ClientImportOperation, Finalizer, ImportSummary, NewBlockState,
		PrunableStateChangesTrieStorage,
	},
	client::{
		BlockBody, BlockImportNotification, BlockOf, BlockchainEvents, ClientInfo,
...
type StorageUpdate<B, Block> =
	<<<B as backend::Backend<Block, Blake2Hasher>>::BlockImportOperation as BlockImportOperation<
		Block,
		Blake2Hasher,
...
impl<H> PrePostHeader<H> {
	...
	// get a reference to the "post-header" -- the header as it should be after all changes are
	// applied.
	fn post(&self) -> &H {
		match *self {
			PrePostHeader::Same(ref h) => h,
	...
	Block: BlockT<Hash = H256>,
{
	...
	pub fn execution_extensions(&self) -> &ExecutionExtensions<Block> { &self.execution_extensions }
	...
	pub fn child_storage(
		...
		Ok(self
			.state_at(id)?
			.child_storage(&child_storage_key.0, &key.0)
			.map_err(|e| sp_blockchain::Error::from_state(Box::new(e)))?
			.map(StorageData))
	}
	...
	pub fn child_storage_hash(
		...
		Ok(self
			.state_at(id)?
			.child_storage_hash(&child_storage_key.0, &key.0)
			.map_err(|e| sp_blockchain::Error::from_state(Box::new(e)))?)
	}
	...
	pub fn code_at(&self, id: &BlockId<Block>) -> sp_blockchain::Result<Vec<u8>> {
		Ok(self
			.storage(id, &StorageKey(well_known_keys::CODE.to_vec()))?
			.expect(
				"None is returned if there's no value stored for the given key;':code' key is \
				 always defined; qed",
			)
			.0)
	}
	...
	pub fn read_proof<I>(&self, id: &BlockId<Block>, keys: I) -> sp_blockchain::Result<StorageProof>
	where
		I: IntoIterator,
		I::Item: AsRef<[u8]>,
	{
	...
	{
		self.state_at(id)
			.and_then(|state| prove_child_read(state, storage_key, keys).map_err(Into::into))
	}
	...
	pub fn header_proof_with_cht_size(
		...
		let proof = cht::build_proof::<Block::Header, Blake2Hasher, _, _>(
			cht_size,
			cht_num,
	...
	pub fn key_changes_proof_with_cht_size(
		...
		impl<'a, Block: BlockT> ChangesTrieRootsStorage<Blake2Hasher, NumberFor<Block>>
			for AccessedRootsRecorder<'a, Block>
		{
			fn build_anchor(
				&self,
				hash: H256,
			) -> Result<ChangesTrieAnchorBlockId<H256, NumberFor<Block>>, String> {
		...
		impl<'a, Block: BlockT> ChangesTrieStorage<Blake2Hasher, NumberFor<Block>>
			for AccessedRootsRecorder<'a, Block>
		{
			fn as_roots_storage(
				&self,
			) -> &dyn state_machine::ChangesTrieRootsStorage<Blake2Hasher, NumberFor<Block>> {
				self
			}
		...
		.map_err(|err| {
			sp_blockchain::Error::from(sp_blockchain::Error::ChangesTrieAccessFailed(err))
		})?;
	...
	/// Generates CHT-based proof for roots of changes tries at given blocks (that are part of
	/// single CHT).
	...
	fn require_changes_trie(
		&self,
	) -> sp_blockchain::Result<(ChangesTrieConfiguration, &B::ChangesTrieStorage)> {
	...
	) -> sp_blockchain::Result<block_builder::BlockBuilder<Block, Self>>
	where
		E: Clone + Send + Sync,
		RA: Send + Sync,
		...
		let storage_changes = match &body {
			Some(body) if enact_state => {
				...
				let (storage_update, changes_update, storage_changes) =
					self.block_execution(&operation.op, &import_headers, origin, hash, &body)?;
				...
				if let Some(storage_changes) = storage_changes.clone() {
					operation
						.op
						.update_storage(storage_changes.0, storage_changes.1)?;
				}
		...
		match transaction.state()? {
			Some(transaction_state) => {
				...
				let get_execution_manager =
					|execution_strategy: ExecutionStrategy| match execution_strategy {
						...
						ExecutionStrategy::Both => {
							ExecutionManager::Both(|wasm_result, native_result| {
								let header = import_headers.post();
								warn!(
									"Consensus error between wasm and native block execution at \
									 block {}",
									hash
								);
								warn!("   Header {:?}", header);
				...
				let encoded_block = <Block as BlockT>::encode_from(import_headers.pre(), body);

				let (_, storage_update, changes_update) = self
					.executor
					.call_at_state::<_, _, NeverNativeValue, fn() -> _>(
						...
						match origin {
							...
							_ => get_execution_manager(
								self.execution_extensions().strategies().importing,
							),
						},
						None,
						None,
	...
	pub fn block(&self, id: &BlockId<Block>) -> sp_blockchain::Result<Option<SignedBlock<Block>>> {
		Ok(
	...
	RA: Send + Sync,
{
	...
	fn info(&self) -> blockchain::Info<Block> { self.backend.blockchain().info() }
...
where
	...
	Block: BlockT<Hash = H256>,
{
	fn cache(&self) -> Option<Arc<dyn Cache<Block>>> { self.backend.blockchain().cache() }
}

impl<B, E, Block, RA> ProvideRuntimeApi for Client<B, E, Block, RA>
where
	B: backend::Backend<Block, Blake2Hasher>,
	...
	RA: ConstructRuntimeApi<Block, Self>,
{
	...
	fn runtime_api<'a>(&'a self) -> ApiRef<'a, Self::Api> { RA::construct_runtime_api(self) }
}
	...
	Block: BlockT<Hash = H256>,
{
	...
	>(
		...
		let (manager, extensions) = self
			.execution_extensions
			.manager_and_extensions(at, context);
...
impl<'a, B, E, Block, RA> consensus::BlockImport<Block> for &'a Client<B, E, Block, RA>
where
	B: backend::Backend<Block, Blake2Hasher>,
	E: CallExecutor<Block, Blake2Hasher> + Clone + Send + Sync,
	Block: BlockT<Hash = H256>,
...
impl<B, E, Block, RA> consensus::BlockImport<Block> for Client<B, E, Block, RA>
where
	B: backend::Backend<Block, Blake2Hasher>,
	E: CallExecutor<Block, Blake2Hasher> + Clone + Send + Sync,
	Block: BlockT<Hash = H256>,
...
impl<B, E, Block, RA> Finalizer<Block, Blake2Hasher, B> for Client<B, E, Block, RA>
where
	B: backend::Backend<Block, Blake2Hasher>,
	E: CallExecutor<Block, Blake2Hasher>,
	Block: BlockT<Hash = H256>,
...
pub(crate) mod tests {
	use super::*;
	use consensus::{BlockImport, BlockOrigin, SelectChain};
	...
	pub fn prepare_client_with_key_changes() -> (
		...
		let remote_client = TestClientBuilder::new()
	...
	fn uncles_with_multiple_forks() {
		...
		// A1 -> A2
		let a2 = client
			.new_block_at(&BlockId::Hash(a1.hash()), Default::default())
			.unwrap()
			.bake()
			.unwrap();
		client.import(BlockOrigin::Own, a2.clone()).unwrap();
		...
		// A2 -> A3
		let a3 = client
			.new_block_at(&BlockId::Hash(a2.hash()), Default::default())
			.unwrap()
			.bake()
			.unwrap();
		...
		// A3 -> A4
		let a4 = client
			.new_block_at(&BlockId::Hash(a3.hash()), Default::default())
			.unwrap()
			.bake()
			.unwrap();
		...
		// A4 -> A5
		let a5 = client
			.new_block_at(&BlockId::Hash(a4.hash()), Default::default())
			.unwrap()
			.bake()
			.unwrap();
		...
		// B2 -> B3
		let b3 = client
			.new_block_at(&BlockId::Hash(b2.hash()), Default::default())
			.unwrap()
			.bake()
			.unwrap();
		...
		// B3 -> B4
		let b4 = client
			.new_block_at(&BlockId::Hash(b3.hash()), Default::default())
			.unwrap()
			.bake()
			.unwrap();
	...
	fn import_with_justification() {
		...
		assert_eq!(client.info().chain.finalized_hash, a3.hash(),);
	...
	fn importing_diverged_finalized_block_should_trigger_reorg() {
		...
		//    -> B1
		let a1 = client
			.new_block_at(&BlockId::Number(0), Default::default())
			.unwrap()
			.bake()
			.unwrap();
		client.import(BlockOrigin::Own, a1.clone()).unwrap();
		...
		assert_eq!(client.info().chain.best_hash, a2.hash(),);
		...
		assert_eq!(client.info().chain.best_hash, b1.hash(),);
	...
	fn finalizing_diverged_block_should_trigger_reorg() {
		...
		let b2 = client
			.new_block_at(&BlockId::Hash(b1.hash()), Default::default())
			.unwrap()
			.bake()
			.unwrap();
		client.import(BlockOrigin::Own, b2.clone()).unwrap();
		...
		assert_eq!(client.info().chain.best_hash, b3.hash(),);
	}
	...
	fn state_reverted_on_reorg() {
		...
		// Re-org to A2
		client
			.import_as_best(BlockOrigin::Own, a2.bake().unwrap())
			.unwrap();
