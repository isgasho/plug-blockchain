...
use sp_runtime::traits::{Header as HeaderT, One, SimpleArithmetic, Zero};
use state_machine::{
	backend::InMemory as InMemoryState, prove_read_on_trie_backend, read_proof_check,
	read_proof_check_on_proving_backend, Backend as StateBackend, MemoryDB, StorageProof,
	TrieBackend,
};
...
pub fn size<N: From<u32>>() -> N { SIZE.into() }
...
pub fn encode_cht_key<N: Encode>(number: N) -> Vec<u8> { number.encode() }

/// Convert header hash into CHT value.
fn encode_cht_value<Hash: AsRef<[u8]>>(hash: Hash) -> Vec<u8> { hash.as_ref().to_vec() }
