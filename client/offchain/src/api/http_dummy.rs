...
use primitives::offchain::{HttpError, HttpRequestId, HttpRequestStatus, Timestamp};
use std::{
	future::Future,
	pin::Pin,
	task::{Context, Poll},
...
pub fn http() -> (HttpApi, HttpWorker) { (HttpApi, HttpWorker) }
...
impl HttpApi {
	...
	pub fn request_add_header(&mut self, _: HttpRequestId, _: &str, _: &str) -> Result<(), ()> {
		unreachable!(
			"Creating a request always fails, thus this function will never be called; qed"
		)
	}
	...
	pub fn response_wait(
		...
		if requests.is_empty() {
			...
			unreachable!(
				"Creating a request always fails, thus the list of requests should always be \
				 empty; qed"
			)
		}
	}
...
impl Future for HttpWorker {
	...
	fn poll(self: Pin<&mut Self>, _: &mut Context) -> Poll<Self::Output> { Poll::Ready(()) }
}
