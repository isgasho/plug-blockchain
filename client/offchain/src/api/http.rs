...
use bytes::Buf as _;
use fnv::FnvHashMap;
use futures::{channel::mpsc, compat::Compat01As03, prelude::*};
use log::error;
...
impl HttpApi {
	...
	pub fn request_write_body(
		...
		let mut poll_sender = move |sender: &mut hyper::body::Sender| -> Result<(), HttpError> {
			let mut when_ready =
				future::maybe_done(Compat01As03::new(futures01::future::poll_fn(|| {
					sender.poll_ready()
				})));
...
mod tests {
	...
	use primitives::offchain::{Duration, HttpError, HttpRequestId, HttpRequestStatus};
	...
	macro_rules! build_api_server {
		() => {{
			...
			std::thread::spawn(move || {
				tokio::run(futures::compat::Compat::new(
					worker.map(|()| Ok::<(), ()>(())),
				))
			});
