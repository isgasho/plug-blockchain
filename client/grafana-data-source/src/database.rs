...
impl Database {
	...
	pub fn truncate(&mut self, new_base_timestamp: i64) -> Result<(), Error> {
		...
		let delta =
			u32::try_from(new_base_timestamp - self.base_timestamp).map_err(Error::Timestamp)?;
...
fn find_index(slice: &[Datapoint], base_timestamp: i64, timestamp: i64) -> usize {
	slice
		.binary_search_by_key(&timestamp, |datapoint| {
...
fn test() {
	...
	assert_eq!(database.keys_starting_with("test ").collect::<Vec<_>>(), [
		"test 2"
	]);
