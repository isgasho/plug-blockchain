...
use sp_runtime::traits::{Block as BlockT, Bounded, CheckedSub, NumberFor, Zero};

use crate::cache::{
	list_entry::{Entry, StorageEntry},
	list_storage::{Metadata, Storage, StorageTransaction},
	CacheItemT, ComplexBlockId, EntryType,
};
...
pub enum CommitOperation<Block: BlockT, T: CacheItemT> {
	...
	BlockFinalized(
		ComplexBlockId<Block>,
		Option<Entry<Block, T>>,
		BTreeSet<usize>,
	),
...
impl<Block: BlockT, T: CacheItemT, S: Storage<Block, T>> ListCache<Block, T, S> {
	...
	pub fn new(
		...
		let (best_finalized_entry, unfinalized) = storage
			.read_meta()
	...
	pub fn value_at_block(
			...
			match self.find_unfinalized_fork(at)? {
				...
				None => match self.best_finalized_entry.as_ref() {
					Some(best_finalized_entry)
						if chain::is_connected_to_block(
							&self.storage,
		...
		match head {
			Some(head) => head
				.search_best_before(&self.storage, at.number)
	...
	pub fn on_block_insert<Tx: StorageTransaction<Block, T>>(
		...
		debug_assert!(
			entry_type != EntryType::Final || self.best_finalized_block.hash == parent.hash
		);
		...
		if !is_final {
			...
			if fork_and_action.is_none() {
				let best_finalized_entry_block = self
					.best_finalized_entry
					.as_ref()
					.map(|f| f.valid_from.number);
				for (index, fork) in self.unfinalized.iter().enumerate() {
					if let Some(action) =
			...
			match fork_and_action {
				...
				Some((index, ForkAppendResult::Append)) => {
					...
					let operation =
						CommitOperation::AppendNewEntry(index, new_storage_entry.into_entry(block));
					tx.update_meta(
						self.best_finalized_entry.as_ref(),
						&self.unfinalized,
						&operation,
				...
				Some((_, ForkAppendResult::Fork(prev_valid_from))) => {
					...
					tx.insert_storage_entry(&block, &new_storage_entry);
					let operation =
						CommitOperation::AddNewFork(new_storage_entry.into_entry(block));
					tx.update_meta(
						self.best_finalized_entry.as_ref(),
						&self.unfinalized,
						&operation,
		...
		match new_storage_entry {
			Some(new_storage_entry) => {
				tx.insert_storage_entry(&block, &new_storage_entry);
				let operation = CommitOperation::BlockFinalized(
					block.clone(),
					Some(new_storage_entry.into_entry(block)),
					abandoned_forks,
				);
				tx.update_meta(
					self.best_finalized_entry.as_ref(),
			...
			None => Ok(Some(CommitOperation::BlockFinalized(
				block,
				None,
				abandoned_forks,
			))),
	...
	fn prune_finalized_entries<Tx: StorageTransaction<Block, T>>(
		...
		let mut do_pruning = || -> ClientResult<()> {
			...
			destroy_fork(first_entry_to_truncate, &self.storage, tx, None)
		};
...
impl<Block: BlockT, T: CacheItemT> Fork<Block, T> {
	...
	pub fn head(&self) -> &Entry<Block, T> { &self.head }
	...
	pub fn try_append_or_fork<S: Storage<Block, T>>(
		...
		if best_finalized_entry_block
			.map(|f| begin.number > f)
			.unwrap_or(true)
		{
...
fn read_forks<Block: BlockT, T: CacheItemT, S: Storage<Block, T>>(
	...
	let finalized = match meta.finalized {
		Some(finalized) => Some(storage.require_entry(&finalized)?.into_entry(finalized)),
		None => None,
	};

	let unfinalized = meta
		.unfinalized
		.into_iter()
		.map(|unfinalized| {
			storage
				.require_entry(&unfinalized)
...
pub mod tests {
	...
	fn list_value_at_block_works() {
		...
		// ----------> [100]
		assert_eq!(
			ListCache::<_, u64, _>::new(
				DummyStorage::new(),
				PruningStrategy::ByDepth(1024),
				test_id(100)
			)
			.value_at_block(&test_id(50))
			.unwrap(),
			None
		);
		...
		assert_eq!(
			ListCache::new(
				DummyStorage::new()
					...
					.with_entry(test_id(100), StorageEntry {
						prev_valid_from: Some(test_id(30)),
						value: 100
					})
					.with_entry(test_id(30), StorageEntry {
						prev_valid_from: None,
						value: 30
					}),
				PruningStrategy::ByDepth(1024),
			...
			.value_at_block(&test_id(50))
			.unwrap(),
			Some((test_id(30), Some(test_id(100)), 30))
		);
		...
		assert_eq!(
			...
			.value_at_block(&test_id(100))
			.unwrap(),
			Some((test_id(100), None, 100))
		);
		...
		assert_eq!(
			ListCache::new(
				DummyStorage::new()
					...
					.with_entry(correct_id(2), StorageEntry {
						prev_valid_from: None,
						value: 2
					})
					.with_entry(correct_id(4), StorageEntry {
						prev_valid_from: Some(correct_id(2)),
						value: 4
					})
			...
			.value_at_block(&fork_id(0, 2, 3))
			.unwrap(),
			Some((correct_id(2), None, 2))
		);
		// when block is later than last finalized block AND there are no matching forks
		...
		assert_eq!(
			...
			.value_at_block(&correct_id(5))
			.unwrap(),
			Some((correct_id(4), None, 4))
		);
	...
	fn list_on_block_insert_works() {
		...
		let mut cache = ListCache::new(
			DummyStorage::new()
				...
				.with_entry(test_id(4), StorageEntry {
					prev_valid_from: None,
					value: 4,
				}),
			PruningStrategy::ByDepth(1024),
		...
		assert_eq!(
			cache
				.on_block_insert(&mut tx, test_id(4), test_id(5), Some(4), nfin)
				.unwrap(),
		...
		assert_eq!(
			cache
				.on_block_insert(&mut tx, test_id(4), test_id(5), Some(5), nfin)
				.unwrap(),
			Some(CommitOperation::AppendNewEntry(0, Entry {
				valid_from: test_id(5),
				value: 5
			}))
		);
		assert_eq!(
			*tx.inserted_entries(),
			vec![test_id(5).hash].into_iter().collect()
		);
		assert!(tx.removed_entries().is_empty());
		assert_eq!(
			*tx.updated_meta(),
		...
		assert_eq!(
			cache
				.on_block_insert(&mut tx, correct_id(4), correct_id(5), Some(4), nfin)
				.unwrap(),
		...
		assert_eq!(
			cache
				.on_block_insert(&mut tx, correct_id(4), correct_id(5), Some(5), nfin)
				.unwrap(),
			Some(CommitOperation::AppendNewEntry(0, Entry {
				valid_from: correct_id(5),
				value: 5
			}))
		);
		assert_eq!(
			*tx.inserted_entries(),
			vec![correct_id(5).hash].into_iter().collect()
		);
		assert!(tx.removed_entries().is_empty());
		assert_eq!(
			*tx.updated_meta(),
		...
		assert_eq!(
			cache
				.on_block_insert(&mut tx, correct_id(3), fork_id(0, 3, 4), Some(14), nfin)
				.unwrap(),
			Some(CommitOperation::AddNewFork(Entry {
				valid_from: fork_id(0, 3, 4),
				value: 14
			}))
		);
		assert_eq!(
			*tx.inserted_entries(),
			vec![fork_id(0, 3, 4).hash].into_iter().collect()
		);
		assert!(tx.removed_entries().is_empty());
		assert_eq!(
			*tx.updated_meta(),
			Some(Metadata {
				finalized: Some(correct_id(2)),
				unfinalized: vec![correct_id(4), fork_id(0, 3, 4)]
			})
		);
		...
		assert_eq!(
			cache
				.on_block_insert(&mut tx, correct_id(2), correct_id(3), Some(2), nfin)
				.unwrap(),
			None
		);
		assert!(tx.inserted_entries().is_empty());
		...
		assert_eq!(
			cache
				...
				.unwrap(),
			Some(CommitOperation::AddNewFork(Entry {
				valid_from: correct_id(3),
				value: 3
			}))
		);
		assert_eq!(
			*tx.inserted_entries(),
			vec![correct_id(3).hash].into_iter().collect()
		);
		assert!(tx.removed_entries().is_empty());
		assert_eq!(
			*tx.updated_meta(),
		...
		assert_eq!(
			cache
				.on_block_insert(&mut tx, correct_id(2), correct_id(3), Some(3), fin)
				.unwrap(),
		...
		assert_eq!(
			cache
				.on_block_insert(&mut tx, correct_id(2), correct_id(3), Some(2), fin)
				.unwrap(),
			Some(CommitOperation::BlockFinalized(
				correct_id(3),
				None,
		...
		let cache = ListCache::new(
			DummyStorage::new()
				...
				.with_entry(fork_id(0, 1, 3), StorageEntry {
					prev_valid_from: None,
					value: 13,
				}),
			PruningStrategy::ByDepth(1024),
	...
	fn list_on_block_finalized_works() {
		...
		let cache = ListCache::new(
			DummyStorage::new()
				...
				.with_entry(correct_id(5), StorageEntry {
					prev_valid_from: Some(correct_id(2)),
					value: 5,
				}),
			PruningStrategy::ByDepth(1024),
		...
		assert_eq!(
			cache
				.on_block_finalize(&mut tx, correct_id(2), correct_id(3))
				.unwrap(),
			Some(CommitOperation::BlockFinalized(
				correct_id(3),
				None,
		...
		assert_eq!(
			cache
				.on_block_finalize(&mut tx, correct_id(4), correct_id(5))
				.unwrap(),
			Some(CommitOperation::BlockFinalized(
				correct_id(5),
				Some(Entry {
	...
	fn list_transaction_commit_works() {
		let mut cache = ListCache::new(
			DummyStorage::new()
				...
				.with_entry(correct_id(6), StorageEntry {
					prev_valid_from: Some(correct_id(5)),
					value: 6,
				}),
			PruningStrategy::ByDepth(1024),
			correct_id(2),
		...
		cache.on_transaction_commit(CommitOperation::AppendNewEntry(0, Entry {
			valid_from: correct_id(7),
			value: 7,
		}));
		assert_eq!(cache.unfinalized[0].best_block, Some(correct_id(7)));
		assert_eq!(cache.unfinalized[0].head, Entry {
			valid_from: correct_id(7),
			value: 7
		});
		// when new fork is added
		cache.on_transaction_commit(CommitOperation::AddNewFork(Entry {
			valid_from: correct_id(10),
			value: 10,
		}));
		assert_eq!(cache.unfinalized[2].best_block, Some(correct_id(10)));
		assert_eq!(cache.unfinalized[2].head, Entry {
			valid_from: correct_id(10),
			value: 10
		});
	...
	fn list_find_unfinalized_fork_works() {
		...
		assert_eq!(
			ListCache::new(
				DummyStorage::new()
					...
					.with_entry(fork_id(0, 1, 3), StorageEntry {
						prev_valid_from: Some(correct_id(1)),
						value: 13
					})
					.with_entry(correct_id(5), StorageEntry {
						prev_valid_from: Some(correct_id(2)),
			...
			.find_unfinalized_fork(&correct_id(4))
			.unwrap()
			.unwrap()
			.head
			.valid_from,
		...
		assert_eq!(
			...
			.find_unfinalized_fork(&fork_id(0, 1, 4))
			.unwrap()
			.unwrap()
			.head
			.valid_from,
		...
		.find_unfinalized_fork(&fork_id(1, 1, 4))
		.unwrap()
		.is_none());
	}
	...
	fn fork_matches_works() {
		...
		let storage = DummyStorage::new()
			...
			.with_entry(test_id(50), StorageEntry {
				prev_valid_from: None,
				value: 50,
			});
		assert_eq!(
		...
		let storage = DummyStorage::new()
			.with_entry(correct_id(5), StorageEntry {
				prev_valid_from: Some(correct_id(3)),
				value: 100,
			})
			.with_entry(correct_id(3), StorageEntry {
				prev_valid_from: None,
				value: 200,
			})
			.with_header(test_header(5))
	...
	fn fork_try_append_works() {
		...
		assert_eq!(
			...
			.try_append(&test_id(101)),
			false
		);
		// when best block is known and the same
		assert_eq!(
			Fork::<_, u64> {
				best_block: Some(test_id(100)),
				head: Entry {
					valid_from: test_id(100),
					value: 0
				}
	...
	fn fork_try_append_or_fork_works() {
		...
		assert_eq!(
			...
			.try_append_or_fork(&storage, &test_id(30), None)
			.unwrap(),
			None
		);
		...
		assert_eq!(
			...
			.try_append_or_fork(&storage, &fork_id(0, 2, 4), None)
			.unwrap(),
			None
		);
		...
		assert_eq!(
			...
			.try_append_or_fork(&storage, &correct_id(6), None)
			.unwrap(),
			Some(ForkAppendResult::Append)
		);
		...
		assert_eq!(
			Fork::<_, u64> {
				best_block: None,
				head: Entry {
					valid_from: correct_id(6),
					value: 100
				}
			}
			.try_append_or_fork(&storage, &fork_id(0, 4, 5), None)
			.unwrap(),
			Some(ForkAppendResult::Fork(ComplexBlockId::new(
				test_header(3).hash(),
				3
			)))
		);
		...
		assert_eq!(
			...
			.try_append_or_fork(&storage, &fork_id(0, 4, 5), Some(3))
			.unwrap(),
			None
		);
	}
	...
	fn fork_destroy_works() {
		...
		.destroy(&storage, &mut tx, Some(200))
		.unwrap();
		assert!(tx.removed_entries().is_empty());
		...
		let storage = DummyStorage::new()
			...
			.with_entry(test_id(50), StorageEntry {
				prev_valid_from: Some(test_id(20)),
				value: 50,
			})
			.with_entry(test_id(20), StorageEntry {
				prev_valid_from: Some(test_id(10)),
				value: 20,
			})
			.with_entry(test_id(10), StorageEntry {
				prev_valid_from: Some(test_id(5)),
				value: 10,
			})
			.with_entry(test_id(5), StorageEntry {
				prev_valid_from: Some(test_id(3)),
				value: 5,
			})
			.with_entry(test_id(3), StorageEntry {
				prev_valid_from: None,
				value: 0,
			});
		let mut tx = DummyTransaction::new();
		...
		assert_eq!(
			*tx.removed_entries(),
			vec![test_id(100).hash, test_id(50).hash, test_id(20).hash]
				.into_iter()
				.collect()
		);
		...
		assert_eq!(
			*tx.removed_entries(),
			vec![test_id(100).hash, test_id(50).hash]
				.into_iter()
				.collect()
		);
	}
	...
	fn is_connected_to_block_fails() {
		...
		assert!(chain::is_connected_to_block::<_, u64, _>(
			&FaultyStorage,
			&test_id(1),
			&test_id(100)
		)
	...
	fn is_connected_to_block_works() {
		...
		assert_eq!(
			chain::is_connected_to_block::<_, u64, _>(
				&DummyStorage::new().with_header(test_header(1)),
				&test_id(1),
				&correct_id(1)
			)
			.unwrap(),
	...
	fn is_finalized_block_works() {
		...
		assert_eq!(
			chain::is_finalized_block::<_, u64, _>(&DummyStorage::new(), &test_id(100), 1).unwrap(),
			false
		);
		// when there's no hash for this block number in the database
		assert_eq!(
			chain::is_finalized_block::<_, u64, _>(&DummyStorage::new(), &test_id(1), 100).unwrap(),
			false
		);
		// when there's different hash for this block number in the database
		assert_eq!(
			chain::is_finalized_block::<_, u64, _>(
				&DummyStorage::new().with_id(1, H256::from_low_u64_be(2)),
				&test_id(1),
				100
			)
			.unwrap(),
		...
		assert_eq!(
			chain::is_finalized_block::<_, u64, _>(
				&DummyStorage::new().with_id(1, H256::from_low_u64_be(1)),
				&test_id(1),
				100
			)
			.unwrap(),
	...
	fn read_forks_works() {
		let storage = DummyStorage::new()
			...
			.with_entry(test_id(20), StorageEntry {
				prev_valid_from: Some(test_id(2)),
				value: 0,
			})
			.with_entry(test_id(30), StorageEntry {
				prev_valid_from: None,
		...
		fn do_test(strategy: PruningStrategy<u64>) {
			...
			match strategy {
				...
				PruningStrategy::ByDepth(_) => {
					assert_eq!(
						*tx.removed_entries(),
						vec![test_id(10).hash].into_iter().collect()
					);
					assert_eq!(
						*tx.inserted_entries(),
	...
	fn revert_block_works() {
		...
		let mut cache = ListCache::new(
			DummyStorage::new()
				.with_meta(Some(correct_id(1)), vec![
					correct_id(5),
					fork_id(1, 2, 5),
					fork_id(2, 4, 5),
				])
				.with_id(1, correct_id(1).hash)
				.with_entry(correct_id(1), StorageEntry {
					prev_valid_from: None,
					value: 1,
				})
				.with_entry(correct_id(3), StorageEntry {
				...
				.with_entry(correct_id(5), StorageEntry {
					prev_valid_from: Some(correct_id(4)),
					value: 5,
				})
				.with_entry(fork_id(1, 2, 4), StorageEntry {
					prev_valid_from: Some(correct_id(1)),
					value: 14,
				})
				.with_entry(fork_id(1, 2, 5), StorageEntry {
					prev_valid_from: Some(fork_id(1, 2, 4)),
					value: 15,
				})
				.with_entry(fork_id(2, 4, 5), StorageEntry {
					prev_valid_from: Some(correct_id(4)),
					value: 25,
				})
				.with_header(test_header(1))
		...
		let op = cache
			.on_block_revert(&mut DummyTransaction::new(), &correct_id(5))
			.unwrap();
		assert_eq!(
			op,
			CommitOperation::BlockReverted(
				vec![(
					0,
					Some(Fork {
						best_block: None,
						head: Entry {
							valid_from: correct_id(4),
							value: 4
						}
					})
				),]
		...
		let op = cache
			.on_block_revert(&mut DummyTransaction::new(), &correct_id(3))
			.unwrap();
		assert_eq!(
			op,
			CommitOperation::BlockReverted(vec![(0, None), (2, None),].into_iter().collect())
		);
		cache.on_transaction_commit(op);

		// when 2 is reverted: entries 4'+5' are truncated
		let op = cache
			.on_block_revert(&mut DummyTransaction::new(), &correct_id(2))
			.unwrap();
		assert_eq!(
			op,
			CommitOperation::BlockReverted(vec![(0, None),].into_iter().collect())
		);
		cache.on_transaction_commit(op);
	}
}
