...
use client_api::backend::{ChildStorageCollection, StorageCollection};
use hash_db::Hasher;
...
impl EstimateSize for Vec<u8> {
	fn estimate_size(&self) -> usize { self.capacity() }
}

impl EstimateSize for Option<Vec<u8>> {
	fn estimate_size(&self) -> usize { self.as_ref().map(|v| v.capacity()).unwrap_or(0) }
}
...
impl<T: EstimateSize> EstimateSize for (T, T) {
	fn estimate_size(&self) -> usize { self.0.estimate_size() + self.1.estimate_size() }
}
...
impl<K: EstimateSize + Eq + StdHash, V: EstimateSize> LRUMap<K, V> {
	...
	fn used_size(&self) -> usize { self.1 }
...
impl<H: Hasher, B: BlockT> CacheChanges<H, B> {
	...
	pub fn sync_cache(
		...
		if let Some(_) = self.parent_hash {
			...
			if is_best {
				trace!(
					"Committing {} local, {} hashes, {} modified root entries, {} modified child \
					 entries",
...
impl<H: Hasher, S: StateBackend<H>, B: BlockT> CachingState<H, S, B> {
	...
	pub fn release(self) -> CacheChanges<H, B> { self.cache }
}
...
mod tests {
	...
	fn smoke() {
		...
		s.cache
			.sync_cache(&[], &[], vec![], vec![], Some(h1a), Some(1), true);
		...
		s.cache
			.sync_cache(&[], &[], vec![], vec![], Some(h3a), Some(3), true);
	...
	fn simple_fork() {
		...
		s.cache
			.sync_cache(&[], &[], vec![], vec![], Some(h2a), Some(2), true);
	...
	fn double_fork() {
		...
		s.cache
			.sync_cache(&[], &[], vec![], vec![], Some(h1), Some(1), true);
		...
		s.cache
			.sync_cache(&[], &[], vec![], vec![], Some(h2b), Some(2), false);
	...
	fn should_track_used_size_correctly() {
		...
		assert_eq!(
			shared.lock().used_storage_cache_size(),
			101 // bytes
		);
	}
	...
	fn fix_storage_mismatch_issue() {
		...
		s.cache
			...
			.write()
			.storage
			.insert(key.clone(), Some(vec![42]));

		// New value is propagated.
		s.cache
			.sync_cache(&[], &[], vec![], vec![], None, None, true);
...
mod qc {
	...
	impl Mutator {
		...
		fn head_state(
			&self,
			hash: H256,
		) -> CachingState<Blake2Hasher, InMemory<Blake2Hasher>, Block> {
			CachingState::new(
				InMemory::<Blake2Hasher>::default(),
				self.shared.clone(),
				Some(hash),
		...
		fn canon_len(&self) -> usize { return self.canon.len() }

		fn head_storage_ref(&self) -> &KeyMap {
			&self.canon.last().expect("Expected to be one commit").state
		}

		fn key_permutations() -> Vec<Vec<u8>> { (0u8..255).map(|x| vec![x]).collect() }

		fn mutate(
			&mut self,
			action: Action,
		) -> Result<CachingState<Blake2Hasher, InMemory<Blake2Hasher>, Block>, ()> {
	...
	fn smoke() {
		...
		mutator.mutate_static(Action::Next {
			hash: h0,
			changes: vec![(key.clone(), Some(vec![2]))],
		});
		mutator.mutate_static(Action::Next {
			hash: h1a,
			changes: vec![],
		...
		mutator.mutate_static(Action::Next {
			hash: h2a,
			changes: vec![(key.clone(), Some(vec![5]))],
		});
		mutator.mutate_static(Action::Next {
			hash: h3a,
			changes: vec![],
		...
		mutator.mutate_static(Action::ReorgWithImport {
			depth: 4,
			hash: h3b,
		});
	...
	fn is_head_match(mutator: &Mutator) -> bool {
		...
		for key in Mutator::key_permutations() {
			match (
				head_state.storage(&key).unwrap(),
				mutator.head_storage_ref().get(&key),
			) {
				(Some(x), Some(y)) => {
					if Some(&x) != y.as_ref() {
						eprintln!("{:?} != {:?}", x, y);
