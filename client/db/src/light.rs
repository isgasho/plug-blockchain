...
use crate::{
	...
	utils::{self, block_id_to_lookup_key, db_err, meta_keys, read_db, read_meta, Meta},
	DatabaseSettings,
};
use client::{
	blockchain::{BlockStatus, Cache as BlockchainCache, Info as BlockchainInfo},
	cht,
	light::blockchain::Storage as LightBlockchainStorage,
};
	...
	Block: BlockT,
{
	...
	#[cfg(test)]
	pub(crate) fn cache(&self) -> &DbCacheSync<Block> { &self.cache }
...
impl<Block: BlockT> LightStorage<Block> {
		...
		if let Some(new_cht_number) = cht::is_build_required(cht::size(), *header.number()) {
			...
			while prune_block <= new_cht_end {
				if let Some(hash) = self.hash(prune_block)? {
					let lookup_key = block_id_to_lookup_key::<Block>(
						&*self.db,
						columns::KEY_LOOKUP,
						BlockId::Number(prune_block),
					)?
					.expect(
						"retrieved hash for `prune_block` right above. therefore retrieving \
						 lookup key must succeed. q.e.d.",
					);
	...
	fn read_cht_root(
		...
		let no_cht_for_block =
			|| ClientError::Backend(format!("CHT for block {} not exists", block));
	...
	Block: BlockT,
{
...

		{
			...
			let cache_ops = cache
				...
				.on_block_insert(
					...
					cache_at,
					if finalized {
						CacheEntryType::Final
					} else {
						CacheEntryType::NonFinal
					},
				)?
				.into_ops();
	...
	fn cache(&self) -> Option<Arc<dyn BlockchainCache<Block>>> { Some(self.cache.clone()) }
}
...
pub(crate) mod tests {
	...
	fn finalized_ancient_headers_are_replaced_with_cht() {
		...
		assert_eq!(
			db.db.iter(columns::HEADER).count(),
			(1 + cht_size + 1) as usize
		);
		assert_eq!(
			db.db.iter(columns::KEY_LOOKUP).count(),
			(2 * (1 + cht_size + 1)) as usize
		);
		...
		assert!(db
			.header_cht_root(cht_size, cht_size + cht_size / 2)
			.is_err());
		assert!(db.changes_trie_cht_root(cht_size, cht_size / 2).is_err());
		assert!(db
			.changes_trie_cht_root(cht_size, cht_size + cht_size / 2)
			.is_err());
	...
	fn authorities_are_cached() {
		...
		fn same_authorities() -> HashMap<well_known_cache_keys::Id, Vec<u8>> { HashMap::new() }
