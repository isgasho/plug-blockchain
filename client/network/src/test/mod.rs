...
use crate::{
	...
	config::{
		build_multiaddr, BoxFinalityProofRequestBuilder, NetworkConfiguration, ProtocolId, Roles,
		TransportConfig,
	},
	protocol::{Context, ProtocolConfig},
	service::TransactionPool,
	specialization::NetworkSpecialization,
	NetworkService, NetworkWorker, ReportHandle,
};
use block_builder::BlockBuilder;
use client::LongestChain;
use client_api::{
	backend::{AuxStore, Backend, Finalizer},
	BlockImportNotification, BlockchainEvents, ClientInfo, FinalityNotification,
...
impl NetworkSpecialization<Block> for DummySpecialization {
	...
	fn on_message(&mut self, _ctx: &mut dyn Context<Block>, _peer_id: PeerId, _message: Vec<u8>) {}
}

pub type PeersFullClient = client::Client<
	test_client::Backend,
	test_client::Executor,
	Block,
	test_client::runtime::RuntimeApi,
>;
pub type PeersLightClient = client::Client<
	test_client::LightBackend,
	test_client::LightExecutor,
	Block,
	test_client::runtime::RuntimeApi,
>;
...
pub struct Peer<D, S: NetworkSpecialization<Block>> {
	...
	imported_blocks_stream:
		Box<dyn Stream<Item = BlockImportNotification<Block>, Error = ()> + Send>,
	finality_notification_stream:
		Box<dyn Stream<Item = FinalityNotification<Block>, Error = ()> + Send>,
}

impl<D, S: NetworkSpecialization<Block>> Peer<D, S> {
	/// Get this peer ID.
	pub fn id(&self) -> PeerId { self.network.service().local_peer_id() }

	/// Returns true if we're major syncing.
	pub fn is_major_syncing(&self) -> bool { self.network.service().is_major_syncing() }
	...
	pub fn num_peers(&self) -> usize { self.network.num_connected_peers() }

	/// Returns true if we have no peer.
	pub fn is_offline(&self) -> bool { self.num_peers() == 0 }
		...
		F: FnMut(BlockBuilder<Block, PeersFullClient>) -> Block,
	{
		...
		for _ in 0..count {
			let builder = full_client
				.new_block_at(&BlockId::Hash(at), Default::default())
				.unwrap();
			...
			let (import_block, cache) = self
				.verifier
				.verify(origin, header.clone(), None, Some(block.extrinsics))
				.unwrap();
			...
			self.network
				.on_block_imported(hash, header, Vec::new(), true);
			at = hash;
		}
	...
	pub fn push_blocks_at(&mut self, at: BlockId<Block>, count: usize, with_tx: bool) -> H256 {
		...
		if with_tx {
			...
			self.generate_blocks_at(at, count, BlockOrigin::File, |builder| {
				builder.bake().unwrap()
			})
		}
	}
	...
	pub fn client(&self) -> &PeersClient { &self.client }
	...
	pub fn blocks_count(&self) -> u64 {
		self.backend
			.as_ref()
			.map(|backend| backend.blocks_count())
			.unwrap_or(0)
	}
}
...
impl TransactionPool<Hash, Block> for EmptyTransactionPool {
	fn transactions(&self) -> Vec<(Hash, Extrinsic)> { Vec::new() }

	fn hash_of(&self, _transaction: &Extrinsic) -> Hash { Hash::default() }
...
impl SpecializationFactory for DummySpecialization {
	fn create() -> DummySpecialization { DummySpecialization }
}

/// Implements `BlockImport` on an `Arc<Mutex<impl BlockImport>>`. Used internally. Necessary to
/// overcome the way the `TestNet` trait is designed, more specifically `make_block_import`
/// returning a `Box<BlockImport>` makes it impossible to clone the underlying object.
struct BlockImportAdapter<T: ?Sized>(Arc<Mutex<Box<T>>>);

impl<T: ?Sized> Clone for BlockImportAdapter<T> {
	fn clone(&self) -> Self { BlockImportAdapter(self.0.clone()) }
}
...
impl<T: ?Sized> Clone for VerifierAdapter<T> {
	fn clone(&self) -> Self { VerifierAdapter(self.0.clone()) }
}
...
pub trait TestNetFactory: Sized {
	...
	fn default_config() -> ProtocolConfig { ProtocolConfig::default() }
	...
	fn add_full_peer_with_states(&mut self, config: &ProtocolConfig, keep_blocks: Option<u32>) {
		...
		self.mut_peers(|peers| {
			...
			let imported_blocks_stream = Box::new(
				client
			...
			let finality_notification_stream = Box::new(
				client
					.finality_notification_stream()
					.map(|v| Ok::<_, ()>(v))
					.compat()
	...
	fn block_until_sync(&mut self, runtime: &mut tokio::runtime::current_thread::Runtime) {
		runtime
			.block_on(futures::future::poll_fn::<(), (), _>(|| {
				Ok(self.poll_until_sync())
			}))
			.unwrap();
	}
...
impl TestNetFactory for TestNet {
	...
	fn from_config(_config: &ProtocolConfig) -> Self { TestNet { peers: Vec::new() } }
	...
	fn peer(&mut self, i: usize) -> &mut Peer<(), Self::Specialization> { &mut self.peers[i] }

	fn peers(&self) -> &Vec<Peer<(), Self::Specialization>> { &self.peers }
...
impl TestNetFactory for JustificationTestNet {
	...
	fn peers(&self) -> &Vec<Peer<Self::PeerData, Self::Specialization>> { self.0.peers() }
