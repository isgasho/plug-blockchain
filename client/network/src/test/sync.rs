...
#[test]
fn ancestry_search_works_when_common_is_one() { test_ancestor_search_when_common_is(1); }

#[test]
fn ancestry_search_works_when_common_is_two() { test_ancestor_search_when_common_is(2); }

#[test]
fn ancestry_search_works_when_common_is_hundred() { test_ancestor_search_when_common_is(100); }
...
fn sync_justifications() {
	...
	assert_eq!(
		net.peer(0)
			.client()
			.justification(&BlockId::Number(10))
			.unwrap(),
		None
	);
	assert_eq!(
	...
	let h1 = net
		.peer(1)
		.client()
		.header(&BlockId::Number(10))
		.unwrap()
		.unwrap();
	let h2 = net
		.peer(1)
		.client()
		.header(&BlockId::Number(15))
		.unwrap()
		.unwrap();
	let h3 = net
		.peer(1)
		.client()
		.header(&BlockId::Number(20))
		.unwrap()
		.unwrap();
		...
		.block_on(futures::future::poll_fn::<(), (), _>(|| {
			...
			for height in (10..21).step_by(5) {
				if net
					.peer(0)
					.client()
					.justification(&BlockId::Number(height))
					.unwrap() != Some(Vec::new())
				{
					return Ok(Async::NotReady)
				}
...
fn own_blocks_are_announced() {
	...
	net.peer(0)
		.generate_blocks(1, BlockOrigin::Own, |builder| builder.bake().unwrap());
