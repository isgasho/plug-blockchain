...
use crate::{
	message::Message,
	protocol::legacy_proto::{LegacyProto, LegacyProtoOut},
};
use codec::{Decode, Encode};
use futures::{future, prelude::*, try_ready};
use libp2p::{
	core::{muxing::StreamMuxerBox, nodes::Substream, transport::boxed::Boxed, ConnectedPoint},
	swarm::{
...
impl NetworkBehaviour for CustomProtoWithAddr {
	...
	fn new_handler(&mut self) -> Self::ProtocolsHandler { self.inner.new_handler() }
...
>{
	...
	fn inject_dial_failure(&mut self, peer_id: &PeerId) { self.inner.inject_dial_failure(peer_id) }
