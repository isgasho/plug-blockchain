...
use crate::{
	config::ProtocolId,
	protocol::legacy_proto::{
		handler::{CustomProtoHandlerIn, CustomProtoHandlerOut, CustomProtoHandlerProto},
...
impl<TSubstream> LegacyProto<TSubstream> {
	...
	pub fn peerset_debug_info(&mut self) -> serde_json::Value { self.peerset.debug_info() }
	...
	fn inject_node_event(&mut self, source: PeerId, event: CustomProtoHandlerOut) {
		match event {
			...
			CustomProtoHandlerOut::ProtocolError {
				is_severe,
				ref error,
			} if !is_severe => debug!(target: "sub-libp2p", "Handler({:?}) => Benign protocol error: {:?}",
					source, error),
