...
use crate::{
	config::Roles,
	message::{self, BlockAttributes, Direction, FromBlock, RequestId},
};
use client_api::{
	...
	B::Header: HeaderT,
{
	...
	fn accept_response(
		...
		let request = match self.remove(peer.clone(), request_id) {
			...
			None => {
				...
				network.report_peer(
					&peer,
					ReputationChange::new_fatal("Invalid remote response"),
				);
		...
		let (retry_count, retry_request_data) = match try_accept(request, &self.checker) {
			...
			Accept::CheckFailed(error, retry_request_data) => {
				...
				network.report_peer(
					&peer,
					ReputationChange::new_fatal("Failed remote response check"),
				);
				network.disconnect_peer(&peer);
				self.remove_peer(peer);
			...
			Accept::Unexpected(retry_request_data) => {
				...
				network.report_peer(
					&peer,
					ReputationChange::new_fatal("Unexpected remote response"),
				);
				network.disconnect_peer(&peer);
				self.remove_peer(peer);
			...
			|request, checker| match request.data {
				RequestData::RemoteHeader(request, sender) => {
					match checker.check_header_proof(&request, response.header, response.proof) {
						...
						Err(error) => {
							Accept::CheckFailed(error, RequestData::RemoteHeader(request, sender))
						},
					}
				},
			...
			|request, checker| match request.data {
				RequestData::RemoteRead(request, sender) => {
					match checker.check_read_proof(&request, response.proof) {
						...
						Err(error) => {
							Accept::CheckFailed(error, RequestData::RemoteRead(request, sender))
						},
					}
				},
	...
	pub fn on_remote_changes_response(
		...
		self.accept_response(
			...
			response.id,
			|request, checker| match request.data {
				RequestData::RemoteChanges(request, sender) => {
					match checker.check_changes_proof(&request, ChangesProof {
						...
						Err(error) => {
							Accept::CheckFailed(error, RequestData::RemoteChanges(request, sender))
						},
					}
				},
				data => Accept::Unexpected(data),
			...
			|request, checker| match request.data {
				RequestData::RemoteBody(request, sender) => {
					let mut bodies: Vec<_> =
						response.blocks.into_iter().filter_map(|b| b.body).collect();
					...
					match checker.check_body_proof(&request, body) {
						...
						Err(error) => {
							Accept::CheckFailed(error, RequestData::RemoteBody(request, sender))
						},
					}
				},
	...
	pub fn is_light_response(&self, peer: &PeerId, request_id: message::RequestId) -> bool {
		self.active_peers
			.get(&peer)
			.map_or(false, |r| r.id == request_id)
	}
	...
	fn send_to(&self, out: &mut impl LightDispatchNetwork<Block>, peer: &PeerId) {
		match self.data {
			RequestData::RemoteHeader(ref data, _) => {
				out.send_header_request(peer, self.id, data.block)
			},
			RequestData::RemoteRead(ref data, _) => {
				out.send_read_request(peer, self.id, data.block, data.keys.clone())
			},
			RequestData::RemoteReadChild(ref data, _) => out.send_read_child_request(
				peer,
				self.id,
				data.block,
			...
			RequestData::RemoteCall(ref data, _) => out.send_call_request(
				peer,
				self.id,
				data.block,
				data.method.clone(),
...
pub mod tests {
	...
	impl<'a, B: BlockT> LightDispatchNetwork<B> for &'a mut DummyNetwork {
		...
		fn disconnect_peer(&mut self, who: &PeerId) { self.disconnected_peers.insert(who.clone()); }

		fn send_header_request(
			&mut self,
			_: &PeerId,
			_: RequestId,
			_: <<B as BlockT>::Header as HeaderT>::Number,
		) {
		}
	...
	fn disconnects_from_timeouted_peer() {
		...
		light_dispatch.active_peers[&peer0].timestamp =
