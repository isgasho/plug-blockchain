...
use libp2p::{
	...
	kad::{
		record::{self, store::MemoryStore},
		GetClosestPeersError, Kademlia, KademliaEvent, Quorum, Record,
	},
	multiaddr::Protocol,
...
impl<TSubstream> DiscoveryBehaviour<TSubstream> {
	...
	pub fn get_value(&mut self, key: &record::Key) { self.kademlia.get_record(key, Quorum::One) }
